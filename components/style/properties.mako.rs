...
//            }
    ...
    ${single_keyword("flex-direction",
                    "row row-reverse column column-reverse",
                    experimental="flex")}
    ${single_keyword("flex-wrap",
                     "nowrap wrap wrap-reverse",
                     experimental="flex")}
    ${single_keyword("justify-content",
                     "flex-start flex-end center space-between space-around",
                     experimental="flex")}
    ${single_keyword("align-items",
                     "stretch flex-start flex-end center baseline",
                     experimental="flex")}
    ${single_keyword("align-self",
                     "auto flex-start flex-end center baseline stretch",
                     experimental="flex")}
    ${single_keyword("align-content",
    ...
    ${integer_type_or_auto("order",
                           ...
                           "Specified(0)",
                           experimental="flex")}
    ${integer_type_or_auto("flex-grow",
                           "u32",
                           "Specified(0)",
                           "value >= 0",
                           experimental="flex")}
    ${integer_type_or_auto("flex-shrink",
                           "u32",
                           "Specified(1)",
                           "value >= 0",
                           experimental="flex")}
    <%self:longhand name="flex-basis" experimental="flex">
        use app_units::Au;
        use cssparser::ToCss;
        use std::fmt;
        use values::computed::Context;
...
pub mod shorthands {
    ...
    <%self:shorthand name="flex"
        ...
        let flex_basis = { input.try(|input| flex_basis::parse(context, input)) };
        let flex_grow = { input.try(|input| flex_grow::parse(context, input)) };
        let flex_shrink = { input.try(|input| flex_shrink::parse(context, input)) };
        let flex_basis = if flex_basis.is_ok() {
            flex_basis
        } else {
            input.try(|input| flex_basis::parse(context, input))
        };
